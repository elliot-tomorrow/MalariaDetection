...
# On constrained/shared CPUs (e.g. free-tier hosting), letting TensorFlow
# spin up many threads can cause contention and make things *slower*, not
# faster. Pin it down before touching the model.
try:
    import tensorflow as tf
    tf.config.threading.set_intra_op_parallelism_threads(1)
    tf.config.threading.set_inter_op_parallelism_threads(1)
except Exception as thread_config_error:
    print(f"Could not configure TF threading (continuing anyway): {thread_config_error}")

model = keras.saving.load_model(MODEL_PATH)
print("Model loaded.")

# Run one dummy prediction now, at startup, instead of on the first real
# request. The very first call to model.predict() often includes one-time
# graph-tracing/setup overhead — better to pay that cost here (where Render
# gives boot time more room) than inside a live request's timeout window.
print("Warming up model with a dummy prediction ...")
_warmup_start = __import__("time").time()
model.predict(np.zeros((1, 180, 180, 3), dtype=np.float32), verbose=0)
print(f"Warmup complete in {__import__('time').time() - _warmup_start:.1f}s.")
